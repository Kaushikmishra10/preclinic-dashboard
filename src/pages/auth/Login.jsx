import React, { useState } from "react";
import { BsEye, BsEyeSlash, BsEnvelope, BsLock } from "react-icons/bs";
import logo from "../../assets/images/preclinic-icon.svg";
import "../../styles/theme.scss";
import "../../styles/Auth.css";

const Login = ({ onLogin, authError, onNavigateSignup }) => {
  const [email,    setEmail]    = useState("");
  const [password, setPassword] = useState("");
  const [showPass, setShowPass] = useState(false);
  const [remember, setRemember] = useState(false);
  const [loading,  setLoading]  = useState(false);

  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      onLogin(email, password);
    }, 800);
  };

  return (
    <div className="auth-bg d-flex align-items-center justify-content-center min-vh-100 p-3">
      <div className="auth-card">

        {/* Logo */}
        <div className="d-flex align-items-center justify-content-center gap-2 mb-4">
          <img src={logo} alt="Preclinic" style={{ width: 36, height: 36 }} />
          <span className="fw-bold" style={{ fontSize: 22, color: "var(--text-primary)", fontFamily: "var(--font)" }}>
            Preclinic
          </span>
        </div>

        {/* Heading */}
        <div className="text-center mb-4">
          <h5 className="fw-bold mb-1" style={{ fontSize: 20, color: "var(--text-primary)", fontFamily: "var(--font)" }}>
            Welcome Back
          </h5>
          <p style={{ fontSize: 13, color: "var(--text-secondary)", fontFamily: "var(--font)" }}>
            Sign in to your account to continue
          </p>
        </div>

        {/* Error message */}
        {authError && (
          <div
            className="rounded-3 px-3 py-2 mb-3 d-flex align-items-center gap-2"
            style={{ background: "#fee2e2", border: "1.5px solid #fca5a5", fontSize: 13, color: "#dc2626", fontFamily: "var(--font)" }}
          >
            <span>⚠</span> {authError}
          </div>
        )}

        {/* Form */}
        <form onSubmit={handleSubmit}>

          {/* Email */}
          <div className="auth-field-wrap mb-3">
            <label className="auth-label">Email Address</label>
            <div className="auth-input-wrap">
              <BsEnvelope className="auth-input-icon" size={15} />
              <input
                type="email"
                className={`auth-input ${authError ? "auth-input-error" : ""}`}
                placeholder="Enter your email"
                value={email}
                onChange={e => setEmail(e.target.value)}
                required
              />
            </div>
          </div>

          {/* Password */}
          <div className="auth-field-wrap mb-3">
            <label className="auth-label">Password</label>
            <div className="auth-input-wrap">
              <BsLock className="auth-input-icon" size={15} />
              <input
                type={showPass ? "text" : "password"}
                className={`auth-input ${authError ? "auth-input-error" : ""}`}
                placeholder="Enter your password"
                value={password}
                onChange={e => setPassword(e.target.value)}
                required
              />
              <button
                type="button"
                className="auth-eye-btn"
                onClick={() => setShowPass(v => !v)}
                tabIndex={-1}
              >
                {showPass ? <BsEyeSlash size={15} /> : <BsEye size={15} />}
              </button>
            </div>
          </div>

          {/* Remember + Forgot */}
          <div className="d-flex align-items-center justify-content-between mb-4">
            <label className="d-flex align-items-center gap-2 auth-remember">
              <input
                type="checkbox"
                checked={remember}
                onChange={e => setRemember(e.target.checked)}
                className="auth-checkbox"
              />
              <span style={{ fontSize: 12, color: "var(--text-secondary)", fontFamily: "var(--font)" }}>
                Remember me
              </span>
            </label>
            <a href="#" className="auth-link" style={{ fontSize: 12 }}>
              Forgot Password?
            </a>
          </div>

          {/* Submit */}
          <button
            type="submit"
            className="auth-submit-btn w-100"
            disabled={loading}
          >
            {loading && <span className="spinner-border spinner-border-sm me-2" role="status" />}
            {loading ? "Signing in..." : "Sign In"}
          </button>

        </form>

        {/* Divider */}
        <div className="auth-divider my-4">
          <span>or continue with</span>
        </div>

        {/* Social buttons */}
        <div className="d-flex gap-3">
          <button className="auth-social-btn flex-grow-1">
            <svg width="18" height="18" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Google
          </button>
          <button className="auth-social-btn flex-grow-1">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="#1877F2">
              <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
            </svg>
            Facebook
          </button>
        </div>

        {/* Signup link */}
        <p className="text-center mt-4 mb-0" style={{ fontSize: 13, fontFamily: "var(--font)", color: "var(--text-secondary)" }}>
          Don't have an account?{" "}
          <a href="#" className="auth-link fw-semibold"
            onClick={e => { e.preventDefault(); onNavigateSignup && onNavigateSignup(); }}>
            Sign Up
          </a>
        </p>

      </div>
    </div>
  );
};

export default Login;